...
#Unilemmas
uni_lemma_map <- read_csv("data/fr_uni_lemmas.csv")    |> 
  mutate(option = pmap(list("french", definition),
                         build_options)) |>
  dplyr::select(language, definition, option, lexical_class) |>
  unnest(option) |>
...
##############################################
adult_speech <- readLines("data/fr_adult_speech_TCOF.txt")
...
adult_tv <- readLines(here("data/fr_adult_tv.txt")) 
...
child_tv <- readLines(here("data/fr_child_tv.txt"))
...
ch_freq_long <- rbind(child_speech, child_book, child_tv, adult_speech, adult_book, adult_tv)  %>%
  dplyr::select(token, source, on_cdi, word_count_norm, word_count, lexical_class, aoa)  
...
save(ch_freq_new, ch_freq_smooth_new, file="word_freqs_fr.Rdata")
