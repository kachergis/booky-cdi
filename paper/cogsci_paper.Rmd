...
en_rot %>%
  kbl(caption = "English principal component rotations.", digits=2, booktabs=T) %>%
  column_spec(2, italic=c(T,T,T,T,T,T,F), color="white", background=spec_color(en_rot[,1])) %>% # PC1
  column_spec(3, bold=c(F,F,T,F,T,F,F), color="white", background=spec_color(en_rot[,2])) %>% # PC2
  column_spec(4, bold=c(T,F,F,F,F,T,F), color="white", background=spec_color(en_rot[,3])) %>%
  column_spec(5, bold=c(F,F,F,T,F,F,F), color="white", background=spec_color(en_rot[,4])) %>%
  column_spec(6, bold=c(F,T,T,F,F,F,F), color="white", background=spec_color(en_rot[,5])) %>%
  column_spec(7, bold=c(T,F,F,T,F,F,F), color="white", background=spec_color(en_rot[,6])) %>%
...
fr_rot %>%
  ...
  column_spec(2, italic=c(T,T,T,T,T,T,F), color="white", background=spec_color(fr_rot[,1])) %>% # PC1
  column_spec(3, bold=c(F,T,T,F,T,F,F), color="white", background=spec_color(fr_rot[,2])) %>% # PC2
  column_spec(4, bold=c(F,F,T,F,F,T,F), color="white", background=spec_color(fr_rot[,3])) %>%
  column_spec(5, bold=c(T,F,F,F,F,T,F), color="white", background=spec_color(fr_rot[,4])) %>%
  column_spec(6, bold=c(T,F,F,T,F,F,F), color="white", background=spec_color(fr_rot[,5])) %>%
  column_spec(7, bold=c(F,T,F,F,T,F,F), color="white", background=spec_color(fr_rot[,6])) %>%
  column_spec(1, color="black", background = "white") %>%
  row_spec(7, color="black", background = "white")
  #kable_styling(latex_options = "striped", stripe_index = 7)
```
...
Given that past research has found that lexical class strongly modulates influences of word frequency, we next examined the interaction of lexical class (LC) with PC1 - PC6 in our regression. 
We also included the number of letters as a predictor (Nletters) to help control for the overall difficulty of each word. 
...
For English, the more complex model was always significantly preferred, including up to the inclusion of PC6 ($R^2 = .58$), while for French, only the inclusion of the first two principal components was justified ($R^2 = .08$). 
Figure 2 shows the coefficient estimates with p<0.05 for both languages.
...
In general, we observed that registers and sources both matter for early word learning. 
...
fr_coef_list = c("PC1","PC2","LCverb","LCverb","PC1:LCadj","PC1:LCfunc","PC1:LCother")
...
f<-coefplot(pca_lc6_fr, add=TRUE, col.pts="blue", coefficients = fr_coef_list, vertical=TRUE, 
