...
```{r, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3.3, fig.height=5.0, set.cap.width=T, num.cols.cap=1, fig.cap = "Significant regression coefficients for predicting CDI AoAs with PCs and lexical class for English (top) and French (bottom). 'func' stands for function words, 'other' stands for words refering to games, routines and people. 'adj' stands for 'adjective'. "}
...
e<-coefplot(pca_lc6, coefficients = en_coef_list, vertical=TRUE, title="English AoA Coefficients",
              col.pts="red", mar=c(1,1,1,1), cex=.8, xlab='', ylab='') + # mar=c(5.5,2.5,2,2), 
  theme_bw() 
f<-coefplot(pca_lc2_fr, add=TRUE, col.pts="blue", coefficients = fr_coef_list, vertical=TRUE, 
              title="French AoA Coefficients", mar=c(1,1,1,1), cex=.8, ylab='', xlab='', offset=.2) + # mar=c(3.5,2.5,2,2),
  theme_bw()
