...
aoa_pc_en <- ddbsa %>% 
  mutate(`Lexical Class` = ifelse(lexical_class=="function_words", "func. word", lexical_class)) %>%
  group_by(`Lexical Class`) %>%
...
#aoa_pc_en %>%
#  papaja::apa_table(digits=2, caption="Correlation of English AoA with PC loadings by LC.", placement='h')
aoa_pc_en <- as.data.frame(aoa_pc_en)
aoa_pc_en %>%
  kbl(caption = "Correlation of English AoA with PC loadings by LC.", digits=2, booktabs=T) %>%
  column_spec(2, color="white", background=spec_color(aoa_pc_en[,2], option="A", end=.8)) %>% # PC1
  column_spec(3, color="white", background=spec_color(aoa_pc_en[,3], option="A", end=.8)) %>% # PC2
  column_spec(4, color="white", background=spec_color(aoa_pc_en[,4], option="A", end=.8)) %>%
  column_spec(5, color="white", background=spec_color(aoa_pc_en[,5], option="A", end=.8)) %>%
  column_spec(6, color="white", background=spec_color(aoa_pc_en[,6], option="A", end=.8)) %>%
  column_spec(7, color="white", background=spec_color(aoa_pc_en[,7], option="A", end=.8)) %>%
  column_spec(1, color="black", background = "white") 
```
