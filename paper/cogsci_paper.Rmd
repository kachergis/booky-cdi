...
PC1 already explains the bulk the variance (89% for English and for French), and PC2-PC4 each only capture an additional 2-4% of the variance. 
In total, the first four components captured >98% of the variance for both languages.
...
For English, PC2 (3.8% of variance) mostly captures child-directed speech, differentiating it from the other registers. 
For example, words such as 'peekaboo' are more present child-directed speech. 
For French, PC2 (6% of variance) mostly captures books for children and adults. 
...
For English, PC3 (3% of variance) captures adult-directed speech, differentiating it from the other registers. It captures words such as 'mower', 'downtown' or 'vitamins', frequent in discussions between adults. 
For French, PC3 (3%) is similar to PC2 in English, capturing child-directed speech, with words such as 'gâteau' = 'sweet'. 
For English, PC4 (2.2% of variance) captures the similarity of child- and adult-directed media, distinguishing them from child- and adult-directed books. 
...
For French, PC4 (2% of variance) captures the similarity of child- and adult-directed media (e.g. 'étoile' = 'star'), distinguishing them from child- and adult-directed speech (e.g. 'voiture' = 'car'). 

For English, PC5 (1% of variance) captures child-directed speech, distinguishing it from child-directed books. 
...
For French, PC5 (3% of variance) distinguishes child-directed media (e.g. 'ferme' = 'farm') from adult-media (e.g. 'café'). 
Lastly, for English, PC6 (<1% of variance) is similar to PC5 in French (1% of variance), capturing child-directed media (.73), and distinguishing it in particular from adult-directed media. 
...
For French, PC6 (1%) distinguishes child-directed books (e.g. 'ours' = 'bear') from adult-directed books (e.g. 'tuer' = 'kill'). 
...
For French, we observe a similar pattern of findings, although the importance and order of the PCs is not quite the same. Variance is similarly distributed across the PCs, but some frequency differences across different registers are more pronounced than for English. 
