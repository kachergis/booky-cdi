...
Table 2 highlights the main differences across the eigenvectors of the principal components (PC1-PC6) in relation to the original six frequency distributions for English and French.
The first PC is similar for both English and French, and captures shared variance between all frequency sources, representing words that are high or low frequency across registers and sources. This component captures for example that 'the' is frequent in spoken and written text and in speech to adults and children. 
For English, PC2 (3.8% of variance) mostly captures child-directed speech, differentiating it from the other registers. For example, motherese words are mostly present in this speech to children. 
For French, PC2 (8% of variance) mostly captures child-directed and adult-directed books. 
This component captures that some words are very frequent in text in general, comparing to the other registers. 

For English, PC3 (3% of variance) captures adult-directed speech, differentiating it from the other registers. For French, PC3 (4%) is similar to PC2 in English, capturing child-directed speech. For English, PC4 (2.2% of variance) captures the similarity of child- and adult-directed media, distinguishing them from child- and adult-directed books. This component captures that some words are very frequent in media in general, but infrequent in books. For French, PC4 (4% of variance) captures the similarity of child- and adult-directed media, distinguishing them from child- and adult-directed speech. 

For English, PC5 (1% of variance) captures child-directed speech, distinguishing it from child-directed books. For French, PC5 (3% of variance) distinguishes child-directed media from adult-media. Last, for English, PC6 (<1% of variance) is similar to PC5 in French, capturing child-directed media (.73), and distinguishing it in particular from adult-directed media. For English, PC6 (3%) distinguishes child-directed from adult-directed books. 
...
EN = c("Freq (-.26...-.52)", "ChS (.65)", "AdS (.79)", "M / B (.40,.48/-.60,-.50)", "ChS / ChB (-.60/.53)", "ChM / AdM (.73/.52)"),
...
## PCA-based Age of Acquisition Regression (Question 2)


Next, we turned to question 2 asking how well these components predict English- and French-learning children's early word learning. We attempted a simple regression predicting each CDI word's mean AoA. However, multicollinearity makes it unwise to include these raw distributions in a regression, as the results would likely be unstable. We verified that this was the case by running a regression predicting AoA with log(word frequency) from each of the six distributions as predictors. 

The Variance Inflation Factor (VIF) estimates the inflation of the variance of a regression coefficient when there is correlation between predictors [@dodge2008concise]. The more the VIF increases, the less reliable the regression results are. The VIF for every distribution was $>>1$ (and many $>5$ or $>10$), indicating that these variables show strong multicollinearity which may compromise the reliability of the regression results. We thus used the CDI items' PCA loadings in lieu of the frequency distributions to predict AoA. Figure 2 shows the loadings of English CDI items on PC1-PC4 vs. the average age of acquisition (AoA; in months).
...
e<-coefplot(pca_lc6, coefficients=c("PC1","PC2","PC3","PC4","PC5",  
                              "LCadj","LCfunc", "LCother", "LCverb", "PC1:LCadj", "PC1:LCfunc", "PC1:LCother", "PC1:LCverb", "PC2:LCother", "PC2:LCother", "PC3:LCfunc", "PC6:LCfunc"), vertical=FALSE, mar=c(5.5,2.5,2,2))
f<-coefplot(pca_lc4_fr, coefficients=c("PC1","PC2","PC3","PC6",  
                              "LCfunc", "LCverb", "PC1:LCadj", "PC1:LCfunc", "PC1:LCverb","PC2:LCverb"), vertical=FALSE, mar=c(5.5,2.5,2,2)) 

ggarrange(e,f, nrow=1, ncol=1, legend = "none")

```

For both languages, lexical categories were significant. For English, P1, P2, P3 and P4 predict the age of acquisition. The differences in word frequency across language sources ar erelevant in predicting AoA. ChS (PC2) is a negative predictor, whereas AdS is a positive predictor. Interactions between P1, with the lexical categories. P2 interacts with other, P3 with functionwords, and P6 with function words. ########add interpretations

For French, P1,P2 and P3 are significant. #### Book coefficient negative, ChS positive? P1 interacts with verb and function words, P2 interacts with verbs.  #######add interpretations
