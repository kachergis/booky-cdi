...
EN = c("Freq -.26...-.52", "ChS .65; AdB -.47", "AdS .79; ChM -.35", "M/B .40.48/-.44-.63", "ChS/ChB -.60/.53", "ChM/AdM .73/-.53"),
FR = c("Freq -.39...-.44", "ChS .65; AdB -.67", "AdS .63; ChM -.69", "M/S .65.23/-.20-.68", "ChM/AdM -.53/.69", "ChB/AdB -.59/.70"))
...
aoa_pc_en <- cor(ddbsa[,"aoa"], ddbsa[,paste0("PC",1:6)]) 
ddbsa_fr_nona <- ddbsa_fr %>% filter(aoa<40, !is.na(aoa)) %>%
  group_by(definition) %>% 
  summarize(aoa=mean(aoa), PC1=mean(PC1), PC2=mean(PC2), PC3=mean(PC3), 
            PC4=mean(PC4), PC5=mean(PC5), PC6=mean(PC6))
  
aoa_pc_fr <- cor(ddbsa_fr_nona[,"aoa"], ddbsa_fr_nona[,paste0("PC",1:6)]) 
aoa_vs_pc <- rbind(aoa_pc_en, aoa_pc_fr)
row.names(aoa_vs_pc) = c("Eng AoA", "Fr AoA")
aoa_vs_pc %>%
  papaja::apa_table(digits=2, caption="Correlation of PC loadings with frequency distributions.", placement='h')
```
