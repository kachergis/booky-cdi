...
After cleaning, this children's media corpus totals `r subset(total_child_tokens, source=="tv")$word_count` tokens across `r subset(total_child_tokens, source=="tv")$n` word types.  For French, we also used openly accessible movie subtitles (https://www.subsynchro.com/), X number of films. After cleaning, this children's media corpus totals `r subset(total_child_tokens_fr, source=="child_tv")$word_count` tokens across `r subset(total_child_tokens_fr, source=="child_tv")$n` word types. 
...
#en_med_reg <- glmer(cbind(total_prod, total_not_prod) ~ age_sc * PC1 + age_sc * PC2 * mom_ed_num + (1|definition), family = binomial(logit), data=en_dat)
# PC1 +.03 PC2 +.19, mom_ed +.06, PC2:mom_ed .01
#summary(en_med_reg)

en2 <- glmer(cbind(total_prod, total_not_prod) ~ age_sc * mom_ed_num * (PC1 + PC2) + (1|definition), family = binomial(logit), data=en_dat)

en3 <- glmer(cbind(total_prod, total_not_prod) ~ age_sc * mom_ed_num * (PC1 + PC2 + PC3) + (1|definition), family = binomial(logit), data=en_dat)

en4 <- glmer(cbind(total_prod, total_not_prod) ~ age_sc * mom_ed_num * (PC1 + PC2 + PC3 + PC4) + (1|definition), family = binomial(logit), data=en_dat)
