...
fr_rot %>%
  ...
  column_spec(2, italic=c(T,T,T,T,T,T,F)) %>% # PC1
  column_spec(3, bold=c(F,F,T,F,F,F,F), italic=c(F,T,F,F,T,F,F)) %>% # PC2
  column_spec(4, bold=c(F,F,F,F,F,T,F), italic=c(F,F,T,F,F,F,F)) %>%
  column_spec(5, bold=c(T,F,F,F,F,F,F), italic=c(F,F,F,F,F,T,F)) %>%
  column_spec(6, bold=c(F,F,F,T,F,F,F), italic=c(T,F,F,F,F,F,F)) %>%
  column_spec(7, bold=c(F,T,F,F,F,F,F), italic=c(F,F,F,F,T,F,F)) %>%
  kable_styling(latex_options = "striped", stripe_index = 7)
```
