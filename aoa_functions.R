...
collapse_inst_data <- function(inst_data) {
  ...
  inst_uni_lemmas <- inst_data |>
    ...
    #group_by(uni_lemma) |> replace unilemmas by definitions
    group_by(definition) |>
    #nest(items = c(lexical_class, category, item_id, definition)) replace unilemmas by definitions
    nest(items = c(lexical_class, category, item_id, uni_lemma))

  inst_data |>
    filter(!is.na(value)) |>
    # for each child and uni_lemma, collapse across items
   # group_by(language, form, measure, uni_lemma, age, data_id) |> replace unilemmas by definitions
    group_by(language, form, measure, definition, age, data_id) |>
    summarise(uni_value = any(value)) |>
    # for each age and uni_lemma, collapse across children
    #group_by(language, form, measure, uni_lemma, age) |>replace unilemmas by definitions
...
combine_form_data <- function(inst_summaries) {
  ...
  inst_combined |>
    unnest(items) |>
    nest(items = -c(language, measure, definition, age, num_true, total)) |>
    #group_by(language, measure, uni_lemma, age) |> replace unilemmas by definition
    group_by(language, measure, definition, age) |>
...
create_wb_data <- function(language, write = TRUE) {
  ...
  if (write) {
    ...
    saveRDS(lang_summary, file = glue("data/{lang_label}.rds"))
  }
  return(lang_summary)
}
...
load_wb_data <- function(languages, cache = TRUE) {
  wb_data <- map_df(languages, function(lang) {
    ...
    lang_file <- glue("data/{norm_lang}.rds")
