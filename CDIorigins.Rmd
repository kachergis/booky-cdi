...
d_samp <- d_samp %>% mutate(books_sc = scale(log(books)),
                            tv_sc = scale(log(tv)),
...
# make mom_ed numeric? 
m1 <- lmer(produces ~ age + books_sc * mom_ed_sc + speech_sc * mom_ed_sc + (1|item_id) + (1|data_id), data=d_samp)
summary(m1)
anova(m1)

m1p <- ggemmeans(m1, c("books_sc", "speech_sc", "mom_ed_sc"))
plot(m1p)

m1pint <- ggemmeans(m1, c("books_sc", "speech_sc", "books_sc:mom_ed_sc", "mom_ed_sc:speech_sc"))
plot(m1pint)

m1pint <- ggemmeans(m1, c("books_sc", "speech_sc", "mom_ed_sc", 
                          "books_sc:mom_ed_sc", "mom_ed_sc:speech_sc"))
m1pred <- ggpredict(m1)
m1eff <- ggeffect(m1)

m2 <- lmer(produces ~ age + lexical_class * books_sc * mom_ed_sc + lexical_class * speech_sc * mom_ed_sc + (1|item_id) + (1|data_id), data=d_samp)
summary(m2)
