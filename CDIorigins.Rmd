...
# ToDo: a small number of values in Dom_PoS_SUBTLEX are numeric...looks like copy/paste errors
...
```{r}
ch_freq <- ch_freq %>% 
  left_join(adult_aoas %>% select(word, lexical_class, Nletters)) %>% # AoA_Kup_lem
  left_join(d_en_ws %>% distinct(definition, lexical_class), by=c("word"="definition")) %>%
  rename(lexical_class = lexical_class.x) %>% # ToDo: look at disagreements on lexical class
...
ch_freq_smooth <- ch_freq_smooth %>% 
  left_join(adult_aoas %>% select(word, lexical_class, Nletters)) %>%
  left_join(d_en_ws %>% distinct(definition, lexical_class), by=c("word"="definition")) %>%
  rename(lexical_class = lexical_class.x) %>% # ToDo: look at disagreements on lexical class
  mutate(lexical_class = ifelse(is.na(lexical_class), lexical_class.y, lexical_class)) %>%
  select(-lexical_class.y)
...
aoas <- aoas %>% left_join(d_en_ws %>% distinct(definition, category, lexical_class))

aoas[which(is.na(aoas$lexical_class)),]
# inside and in don't have category/LC defined
...
ddd <- itdat %>% filter(!is.na(on_cdi)) # 657/680

#summary(lm(mean_produces ~ prop_booky, data=ddd)) # r^2 = .21; more booky = harder

summary(lm(aoa ~ log(speech), data=ddd)) # .01
summary(lm(aoa ~ log(tv), data=ddd))
summary(lm(aoa ~ log(Montag), data=ddd)) # .0003
summary(lm(aoa ~ log(books), data=ddd)) # .03
summary(lm(aoa ~ log(adult_books), data=ddd)) # .03
