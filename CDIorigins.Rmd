...
```{r}
summary(lm(aoa ~ log(tv), data=ddd)) # R^2 = .008
...
summary(lm(aoa ~ log(books), data=ddd)) # .054 (Charlesworth) FB children's corpus
...
summary(lm(aoa ~ lexical_class * log(CHILDES), data=ddd)) # R^2=.37 CHILDES (Eva/George cleaned) - 698 df
...
#pc_bs <- princomp(dddc[,c("books","speech")])
#summary(pc_bs)
#plot(pc_bs$scores[,1],pc_bs$scores[,2], pch=".")

pc_bsa <- princomp(dddc[,c("books","speech","adult_books")])
summary(pc_bsa)
