...
process_text <- function(path) {
  ...
  tab <- table(words[[1]])
  wf <- data_frame(word = names(tab), count = as.numeric(tab)) %>%
    arrange(desc(count))
  print(paste0("loaded ", nrow(wf), " word types (", sum(wf$count)," tokens)"))
  return(wf)
}
...
adult["books"] = process_text(here(paste0(data_dir, "Adult_Books/gutenberg_corpus.txt")))
adult["speech"] = process_text(here(paste0(data_dir, "Adult_Speech/adult_speech_corpus.txt"))) # or lemmatized_adult_speech_corpus.txt
# loaded 27536 word types (3104328 tokens)
adult["tv1"] = process_text(here(paste0(data_dir, "Adult_TV/pbs_adults_corpus.txt")))
# loaded 45351 word types (4175570 tokens)
adult["tv2"] = process_text(here(paste0(data_dir, "Adult_TV/simply_scripts_corpus.txt")))
# loaded 103079 word types (2056384 tokens)

child["books"] = process_text(here(paste0(data_dir, "Child_Books/unlemmatized_child_books.txt")))
# loaded 42652 word types (4730867 tokens)
child["tv"] = process_text(here(paste0(data_dir, "Child_TV/childrens_tv_combined_corpus.txt")))
# loaded 84341 word types (6759928 tokens)
child["speech"] = process_text(here(paste0(data_dir, "CHILDES clean text/corpus_parents.txt")))
# loaded 38770 word types (5495648 tokens)

save(adult, child, file="data/Charlesworth-unlemmatized-counts.Rdata")
```
